...
exports.zbmRouter.get('/leaderboard', async (req, res) => {
    ...
    const rows = await Promise.all(tdrs.map(async (tdr) => {
        const [agents, merchants, visits, floatTotal, floatResolved] = await Promise.all([
            ...
            prisma_1.prisma.agent.count({ where: { tdrId: tdr.id, type: 'merchant', createdAt: { gte: start, lte: end } } }),
            prisma_1.prisma.visit.count({ where: { tdrId: tdr.id, createdAt: { gte: start, lte: end } } }),
            prisma_1.prisma.floatIssue.count({ where: { tdrId: tdr.id, reportedAt: { gte: start, lte: end } } }),
            prisma_1.prisma.floatIssue.count({ where: { tdrId: tdr.id, status: 'resolved', reportedAt: { gte: start, lte: end } } }),
        ]);
