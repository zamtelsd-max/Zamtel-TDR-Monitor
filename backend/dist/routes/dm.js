...
exports.dmRouter.get('/devices', async (req, res) => {
    try {
        ...
        const source = (req.query.source || '').replace(/'/g, "''");
        const status = req.query.status;
        const ase = (req.query.ase || '').replace(/'/g, "''");
        const conds = [];
        if (zone)
            conds.push(`LOWER(zone) = LOWER('${zone}')`);
        if (source)
            conds.push(`"deviceSource" = '${source}'`);
        if (ase)
            conds.push(`"aseName" ILIKE '%${ase}%'`);
        if (status === 'active')
            conds.push(`"activityStatus" = 1`);
        if (status === 'inactive')
            conds.push(`"activityStatus" = 0`);
        if (search)
            conds.push(`("dealerCode" ILIKE '%${search}%' OR imei1 ILIKE '%${search}%' OR ` +
                `"aseName" ILIKE '%${search}%' OR "teamLead" ILIKE '%${search}%' OR ` +
                `zone ILIKE '%${search}%' OR "msisdn" ILIKE '%${search}%')`);
        const where = conds.length ? `WHERE ${conds.join(' AND ')}` : '';
        const [rows, cnt] = await Promise.all([
            prisma_1.prisma.$queryRawUnsafe(`SELECT id,"dealerCode","description","imei1","imei2","msisdn","simSerial",` +
...
exports.dmRouter.post('/devices', async (req, res) => {
    try {
...
exports.dmRouter.patch('/devices/:id', async (req, res) => {
    try {
        const { activityStatus, status, kycReg, grossAdds, zamoGA, recharges, aseName, teamLead } = req.body;
        ...
        if (activityStatus !== undefined)
            updates.push(`"activityStatus" = ${Number(activityStatus)}`);
        if (status !== undefined)
            updates.push(`"status" = '${String(status).replace(/'/g, "''")}'`);
        if (kycReg !== undefined)
            updates.push(`"kycReg" = ${Number(kycReg)}`);
        if (grossAdds !== undefined)
            updates.push(`"grossAdds" = ${Number(grossAdds)}`);
        if (zamoGA !== undefined)
            updates.push(`"zamoGA" = ${Number(zamoGA)}`);
        if (recharges !== undefined)
            updates.push(`"recharges" = ${Number(recharges)}`);
        if (aseName !== undefined)
            updates.push(`"aseName" = '${String(aseName).replace(/'/g, "''")}'`);
        if (teamLead !== undefined)
            updates.push(`"teamLead" = '${String(teamLead).replace(/'/g, "''")}'`);
        await prisma_1.prisma.$queryRawUnsafe(`UPDATE kyc_devices SET ${updates.join(', ')} WHERE id='${req.params.id}'`);
...
exports.dmRouter.delete('/devices/:id', async (req, res) => {
    try {
        const check = await prisma_1.prisma.$queryRaw `SELECT id FROM kyc_devices WHERE id=${req.params.id} LIMIT 1`;
        if (!check.length) {
            res.status(404).json({ error: 'Device not found' });
...
exports.dmRouter.get('/ases', async (req, res) => {
    try {
        const zone = (req.query.zone || '').replace(/'/g, "''");
        const where = zone ? `WHERE LOWER(zone) = LOWER('${zone}')` : '';
        const rows = await prisma_1.prisma.$queryRawUnsafe(`SELECT DISTINCT "aseName", zone FROM kyc_devices ${where} WHERE "aseName" IS NOT NULL ORDER BY zone,"aseName"`);
