...
exports.tdrRouter.post('/agents', async (req, res) => {
    ...
    try {
        ...
        if (existing) {
            const owner = await prisma_1.prisma.users.findUnique({ where: { id: existing.tdrId }, select: { name: true, zone: true, id: true } });
            res.status(409).json({
        ...
        if (err?.code === 'P2002') {
            ...
            try {
                const race = await prisma_1.prisma.agents.findUnique({ where: { agentCode: req.body?.agentCode } });
                const owner = race ? await prisma_1.prisma.users.findUnique({ where: { id: race.tdrId }, select: { name: true, zone: true } }) : null;
                res.status(409).json({
                    error: `Agent code ${req.body?.agentCode} is already registered in the system.`,
                    duplicate: race ? {
                ...
                res.status(409).json({ error: `Agent code already registered.` });
            }
        }
        else {
...
exports.tdrRouter.get('/agents/check-code', async (req, res) => {
    ...
    try {
        ...
        const agent = await prisma_1.prisma.agents.findUnique({ where: { agentCode: code } });
        if (agent) {
            // Find out who owns it
            const owner = await prisma_1.prisma.users.findUnique({ where: { id: agent.tdrId }, select: { name: true, zone: true } });
            res.json({
