      ...
      <div className="divide-y divide-gray-100">
        {zones.map((z) => {
          ...
          return (
            <div key={z.zone}>
              {/* Zone header */}
              <button onClick={() => setOpen(isOpen ? null : z.zone)} className="w-full px-4 py-2.5 flex items-center gap-3 bg-gray-50/60 hover:bg-gray-100/60">
                <span className="text-sm">🗺️</span>
                <div className="flex-1 min-w-0 text-left">
                  <p className="text-sm font-bold text-gray-800 truncate">{z.zone}</p>
                  <p className="text-[10px] text-gray-400">{z.list.length} ASEs · {z.visited}/{z.totalSites} sites visited</p>
                </div>
                <span className="text-sm font-black mr-1" style={{ color: sc(z.avg) }}>{z.avg}%</span>
              ...
              {isOpen && (
                <div className="divide-y divide-gray-50">
                  {z.list.map((r: any, i: number) => (
                    <div key={r.aseId} className="px-4 py-2.5 flex items-center gap-2.5 pl-6">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[10px] text-gray-400">{r.visited}/{r.totalSites} visited · {r.agents}A · {r.ssos}S · {r.odrs}O · {r.zmGa} ZMGA</p>
                      </div>
                      <span className="text-base font-black" style={{ color: sc(r.avgScore) }}>{r.avgScore}%</span>
                    </div>
                  ))}
                </div>
              )}
